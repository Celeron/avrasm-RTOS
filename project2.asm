...
;***** BEGIN "RTOS Tasks" section ******************************************
	...
	; 	   - Если в пуле уже существует Таймер для задачи <tasknumber>, то его текущий "счётчик времени" будет переписан на значение <delay>.	/Прикол с апдейтом таймера кажется лишним, но реально часто пригождается. Например, когда по условию надо отложить событие. Берешь и перезаписываешь таймер, подобно программному вачдогу./
